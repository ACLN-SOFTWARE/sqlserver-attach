...
import { getContainerStatus, startContainer, runNewContainer, copyFilesToContainer, cleanupFiles } from './docker.js';
...
export async function runAttach(targetPath, config, options = {}) {
  ...
  logger.info(`Starting batch attach for ${valid.length} databases with parallel limit ${config.parallel}...`);
  ...
  logger.info(`Operation completed in ${Math.round(totalTime / 1000)}s. Success: ${successCount}, Failed: ${failedCount}`);
