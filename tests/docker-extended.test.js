...
describe('docker extended', () => {
  ...
  it('copyFilesToContainer should resolve on success', async () => {
    ...
    expect(child_process.execSync).toHaveBeenCalledWith(expect.stringContaining('chown mssql:root'));
  });
  ...
  it('cleanupFiles should execute rm command', () => {
    ...
    expect(child_process.execSync).toHaveBeenCalledWith(expect.stringContaining('rm -f "/var/file1"'));
  });
});
